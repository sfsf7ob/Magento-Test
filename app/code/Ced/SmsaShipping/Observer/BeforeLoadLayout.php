...
class BeforeLoadLayout  implements ObserverInterface
{
    ...
    public function __construct(
        \Ced\SmsaShipping\Model\Feed $_feed,
        ...
        $this->_feedHelper = $this->_objectManager->get('Ced\SmsaShipping\Helper\Feed');
    }
    ...
    public function execute(\Magento\Framework\Event\Observer $observer)
    {
       try {
          ...
          //  print_r($this->_objectManager->get('Ced\CsMarketplace\Helper\Feed')->getCedCommerceExtensions(false,true));die;
            ...
            $validateArray=array();
            //$sec=$action->getRequest()->getParam('section');print_r($sec);

            if($this->request->getParam('section') != 'carriers'){ return $this; } 
            $helper = $this->_objectManager->create('Ced\SmsaShipping\Helper\Feed');
            ...
            foreach ($modules as $moduleName=>$releaseVersion)
            {
                ...
                $h = $this->_objectManager->create('Ced\SmsaShipping\Helper\Data')->getStoreConfig(\Ced\SmsaShipping\Block\Extensions::HASH_PATH_PREFIX.$m.'_hash'); 
         
                for($i=1;$i<=(int)$this->_objectManager->create('Ced\SmsaShipping\Helper\Data')->getStoreConfig(\Ced\SmsaShipping\Block\Extensions::HASH_PATH_PREFIX.$m.'_level');$i++)
                    {  
                        $h = base64_decode($h);
                    }

                $h = json_decode($h,true);
                if(is_array($h) && isset($h['domain']) && isset($h['module_name']) && isset($h['license']) && strtolower($h['module_name']) == $m && $h['license'] == $this->_objectManager->create('Ced\SmsaShipping\Helper\Data')->getStoreConfig(\Ced\SmsaShipping\Block\Extensions::HASH_PATH_PREFIX.$m))
                 {}else{
            ...
            foreach ($result as $moduleName => $response) {
                if ($response && isset($response['hash']) && isset($response['level'])) {
                    ...
                    try {
                        ...
                        {
                            $path = \Ced\SmsaShipping\Block\Extensions::HASH_PATH_PREFIX.strtolower($moduleName).'_hash';
                              $this->_feedHelper->setDefaultStoreConfig($path, $response['hash'], 0);
                            $path = \Ced\SmsaShipping\Block\Extensions::HASH_PATH_PREFIX.strtolower($moduleName).'_level';
                             $this->_feedHelper->setDefaultStoreConfig($path, $response['level'], 0);
                            $path = \Ced\SmsaShipping\Block\Extensions::HASH_PATH_PREFIX.strtolower($moduleName);
